...
public class StaggeredGridLayoutManager extends LayoutManager implements ScrollVectorProvider {
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() > 0) {
            ...
            for (lastChildPosition = 0; lastChildPosition < this.mSpanCount; lastChildPosition++) {
                int endLine;
                if (this.mLastLayoutFromEnd) {
                    endLine = this.mSpans[lastChildPosition].getEndLine(Integer.MIN_VALUE);
                    if (endLine != Integer.MIN_VALUE) {
                        endLine -= this.mPrimaryOrientation.getEndAfterPadding();
                    }
                } else {
                    endLine = this.mSpans[lastChildPosition].getStartLine(Integer.MIN_VALUE);
                    if (endLine != Integer.MIN_VALUE) {
    ...
    private int fill(Recycler recycler, LayoutState layoutState, State state) {
        ...
        if (this.mLayoutState.mInfinite) {
            i = layoutState.mLayoutDirection == 1 ? ActivityChooserViewAdapter.MAX_ACTIVITY_COUNT_UNLIMITED : Integer.MIN_VALUE;
